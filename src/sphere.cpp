...
const int inner = slices*spokes*6 + 6*slices;
...
void init(float radius) {
	...
	GLfloat vertices[2*(spokes+1)+slices*spokes][4];

	GLfloat normals[2*(spokes+1)+slices*spokes][3];

	GLfloat tex[2*(spokes+1)+slices*spokes][2];

	GLushort indexes[2*(spokes+2)+inner];
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(vertices)+sizeof(normals)+sizeof(tex), NULL, GL_STATIC_DRAW);
	glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(vertices), vertices);
	glBufferSubData(GL_ARRAY_BUFFER, sizeof(vertices), sizeof(normals), normals);
	glBufferSubData(GL_ARRAY_BUFFER, sizeof(vertices)+sizeof(normals), sizeof(tex), tex);
	...
	texture = loadTexture("checkerboard.jpg");
